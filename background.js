...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  ...
  if (request.action === 'processWithGemini') {
    ...
    processTextWithGemini(request.text)
      .then(result => {
        addLog('✓ Gemini API success', 'success');
...
async function handleClipboardProcess() {
  try {
    ...
    let result;
    try {
      result = await processTextWithGemini(text);
    } catch (error) {
      addLog(`❌ Gemini failed: ${error.message}`, 'error');
...
async function callGeminiAPI(apiKey, model, text) {
  ...
  const response = await fetch(url, {
    ...
    body: JSON.stringify({
      contents: [{
        parts: [{
          text: `You are a helpful academic assistant. Provide responses at a B2 academic level (CEFR), appropriate for a 14-year-old student. Use formal but accessible language. Avoid slang, casual expressions, and em-dashes.
